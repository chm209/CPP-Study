...
```예외를 발생시켰는데 try 블록이 없거나 데이터형이 일치하는 예외 핸들러가 없다면 기본적으로 abort() 함수를 호출한다.```
...
예제 코드에서 주목할 점은, hmean()은 루프를 반복시키고 gmean()은 루프를 종료시킨다.

이것은, 어떤 예외가 발생하는지를 프로그램이 어떻게 결정하고 그 예외에 어떻게 대응하는지를 설명한다.

### 스택풀기

#### c++가 정상적으로 함수 호출을 처리하고 리턴하는 방법

```예제가 길어서 1151,1152 페이지를 직접 보기 바람```

return 구문은 그 함수가 스택에 올려 놓은 객체들만 처리한다. 그러나 throw 구문은 try 블록과 그 throw 구문 사이에 개입된 함수 호출들의 전체적인 연쇄가 스택에 올려 놓은 객체들을 모두 처리한다. (쉽게 말해서 throw를 하면 try 블록안에서 실행한 함수들을 전부 해체하고 catch로 넘어간다. 이것을 스택풀기라고 부른다.)

만약 스택풀기 기능이 없다면, throw 구문은 중간에 개입된 함수 호출들이 스택에 올려 놓은 자동 클래스 객체들에 대해서 파괴자를 호출하지 않고 그대로 스택에 남겨 두게 될 것이다.
...
1. 예외 지정자나 catch 블록이 참조를 지정할지라도 컴파일러는 언제나 예외가 발생할 때 임시 복사본을 만든다는 것이다.
